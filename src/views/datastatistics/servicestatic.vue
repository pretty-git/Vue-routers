  ...
  <div>
    <div style="margin:40px 0 40px 40px;">
      <el-row type="flex" :gutter="10">
        <el-col :xs="30" :sm="30" :md="20" :lg="20" :xl="10">
    ...
    <div style="margin:20px 10%;">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span>{{ scope.row.managerContactWay }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="最近一次修改时间"
...
export default {
  ...
  methods: {
    ...
    async showAll() {
      const cfg = {
        method: 'post',
        url: this.$URL + '/classAdmin/manager/getClassManagerInfoList?pageSize=10&page=' + this.currentpage,
        data: {
    ...
    async handleDelete(index, row) {
      const cfg = {
        method: 'DELETE',
        url: this.$URL + `/classAdmin/manager/delClassManager/${row.classManagerId}`
      }
      var res = await this.$http(cfg)
    ...
    async addnewgrade() { // 转移学生
      ...
      if (this.dialogtitle === '转移学生') {
        ...
        const cfg = {
          method: 'put',
          url: this.$URL + '/classAdmin/manager/transferClass',
          data: list
        }
        var res = await this.$http(cfg)
        if (res.data.data === 'SUCCESS') {
      ...
      if (this.multipleSelection.length === 0) {
        ...
        for (var item of this.multipleSelection) {
          if (this.tableData.includes(item)) {
            list.push(item.classManagerId)
          }
        }
        var lists = list.join(',')
        const cfg = {
          method: 'DELETE',
          url: this.$URL + `/classAdmin/manager/delClassManager/${lists}`
        }
        var res = await this.$http(cfg)
        if (res.data.data === 'SUCCESS') {
          this.$message({
